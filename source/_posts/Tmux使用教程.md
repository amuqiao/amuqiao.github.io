...
```
点击`prefix + I`来获取插件使之生效。

安装成功后使用 `prefix + Ctrl-s` 保存，使用 `prefix + Ctrl-r` 恢复。
...
Tmux 环境默认保存在` ~/.tmux/resurrect dir`中，如果需要更改，使用以下命令
```
# set -g @resurrect-dir '/some/path'
set -g @resurrect-dir '~/.tmux/resurrect'
```
在这个选项中不允许使用环境变量或shell插值，因为字符串是按字面意思使用的。所以下面这些不会达到预期的效果:
```
set -g @resurrect-dir '/path/$MY_VAR/$(some_executable)'
```
仅支持这些变量设置: `$HOME`, `$HOSTNAME`, 和 `~`.

##### mac系统重启后，tmux 插件失效
mac系统重启后，tmux进程终止，tmux 插件失效，`tmux-prefix-highlight`无法高亮显示`prefix`键，使用`tmux`启动进程，`prefix + Ctrl-r`无法恢复`session`

按以下方式可恢复session
1. 重启mac系统
2. 启动tmux进程，`$ tmux`
3. 重载tmux配置文件，`$ prefix + I`
4. 恢复之前保存的session，`prefix + Ctrl-r`
